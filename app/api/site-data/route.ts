// app/api/site-data/route.ts  ← sayo-website (PUBLIC ONLY)
// ─────────────────────────────────────────────────────────
// GET  → serves page data (nav, home, footer, about, services, contact, gallery)
// POST → section=feedback only (public feedback form submission)
//
// Admin routes (POST upsert, PATCH, DELETE) are in sayo-admin project.
// ─────────────────────────────────────────────────────────
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    /* ── feedback (published only — for public reviews page) ── */
    if (section === 'feedback') {
      const page  = Math.max(1, parseInt(req.nextUrl.searchParams.get('page')  ?? '1',  10));
      const limit = Math.min(100, parseInt(req.nextUrl.searchParams.get('limit') ?? '20', 10));
      const skip  = (page - 1) * limit;

      // Public endpoint: only return published reviews
    ...
    return NextResponse.json({
      ...
      footer: footer
        ? { ...footer, locations: Array.isArray(footer.locations) ? footer.locations : FOOTER_DEFAULTS.locations, quick_links: Array.isArray(footer.quick_links) ? footer.quick_links : FOOTER_DEFAULTS.quick_links }
        : FOOTER_DEFAULTS,
      about: about
        ? { ...about, staff: Array.isArray(about.staff) && (about.staff as unknown[]).length > 0 ? about.staff : ABOUT_DEFAULTS.staff, reviews: Array.isArray(about.reviews) && (about.reviews as unknown[]).length > 0 ? about.reviews : ABOUT_DEFAULTS.reviews }
        : ABOUT_DEFAULTS,
      services: (() => {
        if (!services) return SERVICES_DEFAULTS;
        const rawCats = Array.isArray(services.categories) && (services.categories as unknown[]).length > 0 ? services.categories : SERVICES_DEFAULTS.categories;
        const fixedCats = (rawCats as { key: string; label: string; image?: string }[]).map(cat => {
          const def = SERVICES_CATEGORIES_DEFAULT.find(d => d.key === cat.key);
          return { ...cat, image: cat.image && cat.image.startsWith('http') ? cat.image : (def?.image ?? '') };
        });
        return { hero_heading: services.hero_heading || SERVICES_DEFAULTS.hero_heading, hero_subtitle: services.hero_subtitle || SERVICES_DEFAULTS.hero_subtitle, categories: fixedCats, price_list: services.price_list && typeof services.price_list === 'object' ? services.price_list : SERVICES_DEFAULTS.price_list };
      })(),
...
/* ─────────────────────────────────────────
   POST — public feedback form submission ONLY
   (admin upsert POSTs live in sayo-admin)
───────────────────────────────────────── */
export async function POST(req: NextRequest) {
  const section = req.nextUrl.searchParams.get('section');

  if (section !== 'feedback') {
    // All other POST sections (nav, home, footer, about, services, contact, gallery)
    // are handled by sayo-admin. Return 404 here so they don't accidentally hit this endpoint.
  ...
  try {
    ...
    if (Object.keys(validationErrors).length > 0) {
      return NextResponse.json({ error: 'Validation failed', fields: validationErrors }, { status: 422 });
    }

    const feedbackData = { cusName, cusEmail, cusLocation, cusService, cusRating, cusComment, cusConsent };
    ...
    return NextResponse.json({ success: true, message: 'Thank you! Your feedback has been submitted successfully.', id: saved.id }, { status: 201 });

  } catch (err) {
    console.error('[site-data POST feedback]', err);
